...
class ConnectivityLoss(nn.Module):
    ...
    def __init__(self, alpha: float = 0.5, pos_weight: float = 15.0, eps: float = 1e-7):
        super().__init__()
        self.alpha = alpha
    ...
    def forward(self, pred_conn: torch.Tensor, target_conn: torch.Tensor) -> torch.Tensor:
        ...
        # Weight positive connectivity edges (default 15.0 to prevent sparsity zero-collapse)
        pos_weight_tensor = torch.where(target_conn == 1.0, self.pos_weight, 1.0)
        weighted_bce = (bce * pos_weight_tensor).mean()
