...
def plot_loss_curves(history: Dict[str, List[float]], output_path: str):
    """
    Plot and save Training Loss vs Validation Loss over epochs.
    """
    epochs = history.get("epoch", list(range(1, len(history["train_loss"]) + 1)))
    plt.figure(figsize=(9, 5), dpi=150)
    plt.style.use("dark_background")
    plt.plot(epochs, history["train_loss"], "o-", color="#4CAF50", label="Train Loss", linewidth=2.5, markersize=6)
    plt.plot(epochs, history["val_loss"], "s--", color="#FF9800", label="Val Loss", linewidth=2.5, markersize=6)
    plt.title("CadastreVision Training & Validation Loss", fontsize=14, fontweight="bold", pad=12)
    ...
    plt.tight_layout()
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    plt.savefig(output_path, facecolor="#181818", edgecolor="none")
    plt.close()


def plot_metrics_progression(history: Dict[str, List[float]], output_path: str):
    """
    Plot and save F1, IoU, Precision, Recall, and Dice progress over epochs.
    """
    epochs = history.get("epoch", list(range(1, len(history["val_f1"]) + 1)))
    plt.figure(figsize=(10, 5.5), dpi=150)
    plt.style.use("dark_background")
    
    if "val_f1" in history:
        plt.plot(epochs, history["val_f1"], "o-", color="#2196F3", label="Val F1-Score", linewidth=2.5, markersize=6)
    if "val_iou" in history:
        plt.plot(epochs, history["val_iou"], "^-.", color="#00BCD4", label="Val IoU", linewidth=2.0, markersize=6)
    if "val_precision" in history:
        plt.plot(epochs, history["val_precision"], "d--", color="#9C27B0", label="Val Precision", linewidth=1.8, markersize=5)
    if "val_recall" in history:
        plt.plot(epochs, history["val_recall"], "v:", color="#E91E63", label="Val Recall", linewidth=1.8, markersize=5)
    if "val_dice" in history:
        plt.plot(epochs, history["val_dice"], "x--", color="#FFEB3B", label="Val Dice", linewidth=1.5, markersize=5)

    plt.title("CadastreVision Validation Metrics Progression", fontsize=14, fontweight="bold", pad=12)
    plt.xlabel("Epoch", fontsize=11)
